<div class="row">
    @if ($type == 'kendaraan')
        ...
        @endphp
        <div class="col-md-12 ">
            @if (($dPegawaiPeminjamanKendaraan->status ?? 0) == -1 && !empty($dPegawaiPeminjamanKendaraan->verifikator_catatan))
                    ...
                    <div class="d-flex align-items-end gap-2">
                        ...
                        @if (isset($dPegawaiPeminjamanKendaraan->id_peminjaman))
                            @if ($dPegawaiPeminjamanKendaraan->pengembalian_st != 1)
                                ...
                                <a href="#"
                                    class="btn btn-danger btn-sm d-inline-flex align-items-center js-delete"
                ...
                <div class="modal-body">
                        ...
                        <select class="form-select" aria-label="Default select example" name="id_kendaraan"
                            ...
                            @foreach ($dKendaraan as $item)
                                <option value="{{ $item->id_kendaraan }}"
                                    {{ $item->id_kendaraan === ($dPegawaiPeminjamanKendaraan->id_kendaraan ?? '') ? 'selected' : '' }}>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="date" class="form-control"
                            value="{{ $dPegawaiPeminjamanKendaraan->tanggal ?? $tanggal_calendar }}" name="tanggal"
                            readonly>
                        <div class="text-danger"></div>
                    </div>
                    <div class="form-group mb-3 row">
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control " id="jam_mulai_kendaraan" name="jam_mulai"
                                {{ $status != 0 ? 'disabled' : 'required' }}
                                ...
                                data-tanggal="{{ $dPegawaiPeminjamanKendaraan->tanggal ?? $tanggal_calendar }}">
                            <div class="text-danger"></div>
                            <div class="text-danger" id="jam_mulai_kendaraan_error"></div>
                        </div>
                        <div class="col-6">
                            <label class="form-label">Jam Selesai</label>
                            <input type="text" class="form-control" id="jam_selesai_kendaraan" name="jam_selesai"
                                {{ $status != 0 ? 'disabled' : 'required' }}
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control"
                            value="{{ $dPegawaiPeminjamanKendaraan->driver ?? '' }}" name="driver"
                            {{ $status != 0 ? 'disabled' : '' }}>
                        <div class="text-danger"></div>
                    </div>
                    {{-- <div class="form-group mb-3">
                    ...
                    @php
                        $opsi = ['Ukur dan Survey Tanah', 'Cek Keadaan Tanah', 'Menghadiri dari Dinas terkait'];
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="d-flex flex-column gap-1">
                            @foreach ($opsi as $label)
                                <label class="form-check">
                                    <input class="form-check-input keperluan-radio" type="radio"
                                        name="keperluan_option" value="{{ $label }}"
                                        {{ $isPreset && $keperluanAwal === $label ? 'checked' : '' }}
                                        {{ $status != 0 ? 'disabled' : '' }}>
                        ...
                        style="{{ ($isPreset || $keperluanAwal==='') ? 'display:none' : '' }}" --}}>
                            <textarea class="form-control" id="keperluan_lain" rows="4" name="keperluan_lain"
                                placeholder="Tuliskan keperluan lainnya..." {{ $status != 0 ? 'disabled' : '' }}>{{ !$isPreset ? $keperluanAwal : '' }}</textarea>
                        </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea class="form-control" name="detail_lokasi" cols="10" rows="2"
                            {{ $status != 0 ? 'disabled' : '' }}>{{ $dPegawaiPeminjamanKendaraan->detail_lokasi ?? '' }}</textarea>
                        <div class="text-danger"></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary {{ $status != 0 ? 'd-none' : '' }}"
                        id="btnSubmit">
                        <svg class="pc-icon">
    ...
    @else
        ...
        <div class="col-md-12 ">
            ...
            <form action="#" id="formPegawaiPeminjamanRuangan" method="POST" class="form-validate is-alter"
                enctype="multipart/form-data">
                <div class="d-flex justify-content-end align-items-center m-2 gap-2">
                    <a class="btn btn-success btn-sm text-white d-inline-flex align-items-center" href="#!"
                        data-modal data-title="Cek Ketersediaan"
                        data-url="{{ url('/getKetersediaan/ruangan/' . $tanggal_calendar) }}">
                        <i class="fas fa-list me-1"></i> Cek Ketersediaan
                    </a>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" name="id_ruangan"
                            ...
                            @foreach ($dRuangan as $ruangan)
                                <option value="{{ $ruangan->id_ruangrapat }}"
                                    {{ $ruangan->id_ruangrapat ===
                    ...
                    <div class="form-group mb-3">
                        <label class="form-label">Tanggal Peminjaman</label>
                        <input type="date" class="form-control"
                            value="{{ $dPegawaiPeminjamanRuangan->tanggal ?? $tanggal_calendar }}" name="tanggal"
                            readonly>
                        <div class="text-danger"></div>
                    </div>
                    <div class="form-group mb-3 row">
                        <div class="col-6">
                            <label class="form-label">Jam Mulai </label>
                            <input type="text" class="form-control" id="jam_mulai_ruangan" name="jam_mulai"
                                {{ $status != 0 ? 'disabled' : 'required' }}
                                value="{{ !empty($dPegawaiPeminjamanRuangan->jam_mulai)
                                    ? Carbon::parse($dPegawaiPeminjamanRuangan->jam_mulai)->format('H:i')
                                    : '' }}"
                                {{-- Validasi Inputan Jam  --}}
                                data-tanggal="{{ $dPegawaiPeminjamanRuangan->tanggal ?? $tanggal_calendar }}">
                            <div class="text-danger"></div>
                            <div class="text-danger" id="jam_mulai_ruangan_error"></div>
                        </div>
                        <div class="col-6">
                            <label class="form-label">Jam Selesai</label>
                            <input type="text" class="form-control" id="jam_selesai_ruangan" name="jam_selesai"
                                {{ $status != 0 ? 'disabled' : 'required' }}
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" class="form-control"
                            value="{{ $dPegawaiPeminjamanRuangan->jumlah_peserta ?? 0 }}" name="jumlah_peserta"
                            min="1" max="100" oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                            {{ $status != 0 ? 'disabled' : '' }}>
                        <div class="text-danger"></div>
                    </div>
                    @php
                        $opsi1 = ['Rapat Tim Divisi', 'Sosialisasi', 'Forum Koordinasi', 'Pelatihan dan Diklat'];
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="d-flex flex-column gap-1">
                            @foreach ($opsi1 as $label)
                                <label class="form-check">
                                    <input class="form-check-input keperluan-radio" type="radio"
                                        name="keperluan_option" value="{{ $label }}"
                                        {{ $isPreset1 && $keperluanAwal1 === $label ? 'checked' : '' }}
                                        {{ $status != 0 ? 'disabled' : 'required' }}>
                                    <span class="form-check-label">{{ $label }}</span>
                                </label>
                            @endforeach
                            ...
                            <label class="form-check">
                                <input class="form-check-input keperluan-radio" type="radio"
                                    name="keperluan_option" value="__LAIN__"
                                    {{ !$isPreset1 && $keperluanAwal1 !== '' ? 'checked' : '' }}
                                    {{ $status != 0 ? 'disabled' : 'required' }}>
                                <span class="form-check-label">Lain-lain</span>
                            </label>
                        </div>
                        ...
                        style="{{ ($isPreset1 || $keperluanAwal1==='') ? 'display:none' : '' }}" --}}
                            <textarea class="form-control" id="keperluan_lain" rows="4" name="keperluan_lain"
                                placeholder="Tuliskan keperluan lainnya..." {{ $status != 0 ? 'disabled' : '' }}>{{ !$isPreset1 ? $keperluanAwal1 : '' }}</textarea>
                        </div>
...
<script>
    ...
    function validateJamMinimal(selectorInput, selectorError = null, label = 'Jam Mulai') {
        const input = document.querySelector(selectorInput);
        const errorEl = selectorError ? document.querySelector(selectorError) : null;

        if (!input) return true;

        const tanggalPengajuan = input.dataset.tanggal || '';
        const jamInput = (input.value || '').trim();
        ...
        if (tanggalPengajuan === today && jamInput < nowHm) {
            input.classList.add('is-invalid');

            if (errorEl) {
                errorEl.textContent = `Untuk tanggal hari ini, '${label}' minimal ${nowHm}.`;
            }
    ...
    $(document).on('change keyup', '#jam_mulai_kendaraan', function() {
        validateJamMinimal('#jam_mulai_kendaraan', '#jam_mulai_kendaraan_error', 'Jam Mulai');
    });
    $(document).on('change keyup', '#jam_mulai_ruangan', function() {
        validateJamMinimal('#jam_mulai_ruangan', '#jam_mulai_ruangan_error', 'Jam Mulai');
    });
    ...
    $("#formPegawaiPeminjamanKendaraan").submit(function(event) {
        ...
        const url = "{{ route('pegawai-peminjaman.update', ':id') }}".replace(':id',
            idPeminjaman); // update route
        formData.append('_method', 'PUT'); // method PUT

        // validasi jam
        if (!validateJamMinimal('#jam_mulai_kendaraan', '#jam_mulai_kendaraan_error', 'Jam Mulai')) {
            $('#jam_mulai_kendaraan').focus();
    ...
    $("#formPegawaiPeminjamanRuangan").submit(function(event) {
        ...
        // validasi jam
        if (!validateJamMinimal('#jam_mulai_ruangan', '#jam_mulai_ruangan_error', 'Jam Mulai')) {
            $('#jam_mulai_ruangan').focus();
            $("#btnSubmit").prop("disabled", false);
            $("#btnSubmit").html("Simpan");
            return false;
        }
