            ...
            <form action="{{ route('filter.save') }}" method="post" autocomplete="off" class="mb-2">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="pengembalian_st" class="form-control" id="">
                            <option value="" {{ isset($filter['pengembalian_st'])?'':'selected' }}>-- Semua Status --
                            </option>
                            <option value="1" {{ ($filter['pengembalian_st']??'')==1?'selected':'' }}>Sudah Dikembalikan
                            </option>
                            <option value="0" {{ ($filter['pengembalian_st']??'')==0?'selected':'' }}>Belum
                                Dikembalikan
                            </option>
                        </select>
                        @error('pengembalian_st')<small class="text-danger">{{ $message }}</small>@enderror
                    </div>
                </div>
    ...
    <section id="section-data-table-kendaraan">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="mb-0">Laporan Pengembalian Kendaraan</h5>
                <form class="d-inline-flex align-items-center"
                    action="{{ route('satpam.laporan.pengembalian.export_kendaraan') }}" method="post" target="_blank">
                    @csrf
                    <button class="btn btn-success"
                        formaction="{{ route('satpam.laporan.pengembalian.export_kendaraan') }}">
                        <svg class="pc-icon" style="width:14px; height:14px; fill:currentColor;">
