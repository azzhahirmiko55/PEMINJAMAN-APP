        ...
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <form class="d-inline-flex align-items-center"
                    action="{{ ($user->id_role == 2)?route('staff.pelaporan.peminjaman.export_ruangan'):route('admin.pelaporan.peminjaman.export_ruangan') }}"
                    method="post" target="_blank">
                    @csrf
                    @php
                    @endphp
                    <button class="btn btn-danger"
                        formaction="{{ ($user->id_role == 2)?route('staff.pelaporan.peminjaman.export_ruangan'):route('admin.pelaporan.peminjaman.export_ruangan') }}">
                        <svg class="pc-icon" style="width:14px; height:14px; fill:currentColor;">
    ...
    <section id="section-data-table-kendaraan" class="{{ ($filter['section_view']??'')==-1?'':'d-none' }}">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <form class="d-inline-flex align-items-center"
                    action="{{ ($user->id_role == 2)?route('staff.pelaporan.peminjaman.export_kendaraan'):route('admin.pelaporan.peminjaman.export_kendaraan') }}"
                    method="post" target="_blank">
                    @csrf
                    <button class="btn btn-danger"
                        formaction="{{ ($user->id_role == 2)?route('staff.pelaporan.peminjaman.export_kendaraan'):route('admin.pelaporan.peminjaman.export_kendaraan') }}">
                        <svg class="pc-icon" style="width:14px; height:14px; fill:currentColor;">
                            <use xlink:href="#file-pdf"></use>
                        </svg>&nbsp;
                        Export Data
