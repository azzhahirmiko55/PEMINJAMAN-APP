...
function renderDateBadges(calendar) {
    ...
    calendar.getEvents().forEach((ev) => {
        ...
        if (
            !statusByDateType[d][tipe] ||
            PRIORITY(status) > PRIORITY(statusByDateType[d][tipe])
        ) {
    ...
    Object.entries(counts).forEach(([dateStr, { kendaraan, ruangan }]) => {
        ...
        if (kendaraan > 0) {
            wrap.appendChild(
                makeBadge(badgeClsK, `${ICONS.kendaraan} Kendaraan`)
            );
        }

        if (ruangan > 0) {
            wrap.appendChild(makeBadge(badgeClsR, `${ICONS.ruangan} Ruangan`));
        }
...
async function showInfoPeminjaman(ds) {
    ...
    try {
        const res = await $.getJSON("/getDataPegawaiPeminjaman", {
            tanggal: ds,
        });
