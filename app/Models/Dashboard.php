...
class Dashboard
{
    ...
    public static function query_total_peminjaman()
    {
        ...
        $query_kendaraan = DB::table('peminjaman_kendaraan')
                            ...
                            ->select('peminjaman_kendaraan.*', DB::raw('CONCAT(kendaraan.plat, " - ", kendaraan.keterangan) as ket_kendaraan, kendaraan.jenis, kendaraan.warna, kendaraan.plat, kendaraan.keterangan'))->where('peminjaman_kendaraan.status',2)->orWhere('peminjaman_kendaraan.status','=','9');

        $query_ruangrapat = DB::table('peminjaman_ruangrapat')
                                ->join('ruangrapat', 'ruangrapat.id', '=', 'peminjaman_ruangrapat.id_ruangrapat')
                                ->select('peminjaman_ruangrapat.*','ruangrapat.ruangan','ruangrapat.warna',DB::raw('CONCAT(peminjaman_ruangrapat.jam_mulai, " - ", peminjaman_ruangrapat.jam_selesai) as jam'))->where('peminjaman_ruangrapat.status',2);
        ...
        $kendaraan_available = DB::table('kendaraan')
                                ...
                                ->whereNotExists(function ($query) {
                                    $query->select('peminjaman_kendaraan.id')
                                            ...
                                            ->where('peminjaman_kendaraan.status', '=', '2')->orWhere('peminjaman_kendaraan.status','=','9')
        ...
        $ruangrapat_available = DB::table('ruangrapat')
                                ...
                                ->whereNotExists(function ($query) {
                                    $query->select('peminjaman_ruangrapat.id')
                                            ...
                                            ->where('peminjaman_ruangrapat.status', '=', '2')
        ...
        return [
            ...
            'total_kendaraan_bulan' => $total_peminjaman_kendaraan_bulan->where(['status' => 2])->orWhere(['status' => 9])->whereMonth('tanggal', date('m'))->count(),
            'total_kendaraan'       => $total_peminjaman_kendaraan->where(['status' => 2])->orWhere(['status' => 9])->whereYear('tanggal', date('Y'))->count(),
            'total_kendaraan_hari'  => $total_peminjaman_kendaraan_hari->where(['status' => 2])->orWhere(['status' => 9])->whereDate('tanggal', date('Y-m-d'))->count(),

            'list_ruangrapat'        => $query_ruangrapat->orderBy('created_at', 'DESC')->take(5)->get(),
            'ruangrapat_available'   => $ruangrapat_available->get(),
            'total_ruangrapat_bulan'    => $total_peminjaman_ruangrapat_bulan->where(['status' => 2])->whereMonth('tanggal', date('m'))->count(),
            'total_ruangrapat'          => $total_peminjaman_ruangrapat->where(['status' => 2])->whereYear('tanggal', date('Y'))->count(),
            'total_ruangrapat_hari'     => $total_peminjaman_ruangrapat_hari->where(['status' => 2])->whereDate('tanggal', date('Y-m-d'))->count(),
        ];
    }
}
